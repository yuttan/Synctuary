...
Synctuaryでは、ローカルネットワーク外からサーバーにアクセスする方法を3つサポートしています。いずれも同じアプリケーション層プロトコルを使用しており、TCP接続の確立方法だけが異なります。

概要
--------

| | Tailscale（推奨） | IPv6ダイレクト | WireGuard VPN |
|---|---|---|---|
| 仕組み | マネージド型WireGuardメッシュ。サーバーに `100.x` のtailnetアドレスが付与される | サーバーのIPv6 GUAへのTLS接続 | 自己ホスト型の暗号化UDPトンネル |
| 要件 | サーバーとクライアント双方にTailscale | ISPからのIPv6 + ルーターFW開放 | ルーターでのUDPポートフォワーディング |
| CGNAT背後で動作するか？ | はい | いいえ（グローバルIPv6が必要） | ポート開放できる場合のみ |
| ルーター/ISP設定 | **不要** | ファイアウォール設定 + ISPのIPv6 | UDPポートフォワーディング |
| クライアント設定 | Tailscaleを導入しtailnet URLを入力 | IPv6 URLを入力するだけ | 設定のインポート + トンネル有効化 |
| Synctuary側の設定 | **不要** | `remote_access.mode: ipv6` | `remote_access.mode: wireguard` |
| 外部サービス依存 | Tailscaleアカウント（無料プランあり） | なし | なし |

モードの選択
---------------

- **Tailscale** — 推奨のデフォルト。NAT越えを代行してくれるため、ISPのポート制限・CGNAT・ONUやルーターの機能制限を気にする必要がなくなります。Synctuary側の設定変更は一切不要で、Tailscaleがホストにネットワークインターフェースを追加し、アプリは通常のアドレスと同じように接続します。トレードオフはTailscaleのコーディネーションサービスへの依存です（家庭用途なら無料の個人プランで十分。その部分まで自己ホストしたい場合は、オープンソースの互換実装 [Headscale](https://github.com/juanfont/headscale) を利用できます）。
- **IPv6ダイレクト** — 外部サービスに依存せず、レイテンシも最小。ただし実際のIPv6 GUAと、開放可能なルーター/ファイアウォールが必要です。日本の主要ISP（NTTフレッツ/NGN、auひかり、NURO光）はIPoEまたはMAP-EでIPv6 GUAを提供しています。
- **WireGuard VPN** — 完全に自己ホストするトンネル（サーバー内蔵・ユーザースペース実装のためカーネルモジュール不要）。UDPポートの開放が前提となるため、CGNAT環境では利用できません。

> **ライセンスに関する補足**: Tailscaleのクライアントおよび `tsnet` ライブラリはBSD-3-Clauseで、SynctuaryのApache-2.0と互換性があります。SynctuaryはTailscaleのコードを同梱・再配布しておらず（利用者が個別に導入する形）、本ガイドで推奨することによるライセンス上の義務は双方に発生しません。
...
Tailscaleはデバイス間にプライベートなWireGuardメッシュを構築し、NAT越えを自動的に処理します。ルーターの設定変更やポート開放は不要です。

サーバー側の設定
----------------

1. Synctuaryを動かしているマシンにTailscaleを導入し（[ダウンロード](https://tailscale.com/download)）、サインインします。
...
# Windows: MSIをインストール後、タスクトレイのアイコンからサインイン
```

2. ホストのtailnetアドレスを確認します。

```sh
tailscale ip -4        # 例: 100.104.8.74
```

同じ情報は管理画面からも取得できます。`GET /admin/api/tailscale/status` が検出されたアドレスと、そのまま使えるURLを返します。
...
3. これ以外の作業はありません。Synctuary側の設定変更は**不要**です。トランスポートを提供しているのはTailscaleであるため、`remote_access.mode` は `disabled` のままにしてください。

クライアント側の設定（Android）
------------------------------

1. Tailscaleアプリを導入し、サーバーと同じアカウントでサインインして同一tailnetに参加させます。
2. Synctuaryの **設定 → リモートURLを追加** で、サーバーのtailnet URL（`https://100.x.y.z:8443`）を入力します。管理画面のペアリングページのQRコードを読み取る方法でも構いません。
3. 外出時はそのリモートエントリに切り替え、自宅LANではHomeに戻します。両方のエントリは共存できます。

補足
----

- **TLSは引き続き有効です**。tailnet自体が暗号化されていますが、SynctuaryはTLSと証明書ピンニングをそのまま使用します。自動生成される証明書のSANにはLAN側アドレスが含まれるため、tailnetアドレスは含まれていない場合がありますが、クライアントはホスト名ではなく証明書フィンガープリント（§3.3）で検証するため問題ありません。
- **ファイアウォール**: ルーター側のインバウンド設定は不要です。ホストのファイアウォールでは、Tailscaleインターフェース（`tailscale0` / 「Tailscale」）に対してTCP 8443の許可が必要な場合があります。
- **MagicDNS**: 有効にしている場合は、数値アドレスの代わりに `https://<マシン名>:8443` でアクセスできます。
- 家庭内の一般的な構成であればTailscaleの無料個人プランで足ります。多数の端末を追加する場合は最新のプラン条件を確認してください。
