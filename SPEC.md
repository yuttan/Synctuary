...
| コンポーネント | 技術 | ライセンス | 配布 | ステータス (2026-07-10) |
|:---|:---|:---|:---|:---|
| Server | Go 1.22 (single binary, pure Go SQLite) | Apache-2.0 | GitHub OSS + GHCR コンテナ + Windows インストーラー | **v0.7.10** (remote access, サムネイル, ffmpeg トランスコード, アーカイブブラウジング, TLS自動生成) |
| Android Client | Kotlin 2.0 + Jetpack Compose + Material 3 | Apache-2.0 | TBD (Google Play / F-Droid 検討中) | **v0.7.18** (全フェーズ完了 + アーカイブブラウザ, ピンチズーム, トランスコードフォールバック再生, レジューム) |
| iOS Client | (未着手) | TBD | TBD | スタブ |
| Windows Client | (未着手、ただしサーバーはWindowsインストーラーで配布済み) | TBD | TBD | 未計画 |
| Protocol Spec | Markdown | Apache-2.0 (本文)、技術解釈は無償公開 | GitHub | **v0.3.2 確定** (§10 Shares, §11 Pins, §6.6-§6.8 トランスコード/サムネイル/メディア情報, §6.9-§6.11 アーカイブ) |
...
- ファイルは平文でサーバーに保管
- 通信は TLS で保護 (PROTOCOL §12 Transport Security, `tls-ca-verified` / `tls-self-signed`)
...
1. **LAN Mode**（デフォルト `mode: "disabled"`）: 同一 LAN 内で直接接続、自己署名証明書 + §12.1 fingerprint pinning。Android `network_security_config.xml` で対象ホストのみ許可
...
| v0.7 | 写真自動バックアップ、サムネイル生成、TLS自動生成 + シードフレーズUI、Windowsインストーラー、ffmpeg トランスコードストリーミング、アーカイブブラウジング（zip/rar/7z）、コミックリーダービューア | ✅ 完了 (v0.7.10、PROTOCOL v0.3.2) |
| — | 実機ペアリングテスト（サーバー起動済み端末での §4 フロー確認）、サードパーティテスター向けインストーラー配布 | **次** |
...
**Version**: 0.7.10 (PROTOCOL v0.3.2 対応)
**Date**: 2026-07-10
**Status**: Server v0.7.10 (remote access, サムネイル, トランスコード, アーカイブ含む) / Android v0.7.18 全フェーズ完成
