  ...
  return (
    <div>
      <div class="flex items-center justify-between mb-6">
        <h2 class="text-2xl font-bold text-white">Shared Drives</h2>
        <button
            ...
            <div key={s.id} class="bg-gray-900 border border-gray-800 rounded-xl p-5 flex items-center gap-4">
              ...
              <div class="flex-1 min-w-0">
                <div class="flex items-center gap-2">
                  ...
                  {s.is_default && (
                    <span class="text-xs bg-gray-800 text-gray-400 px-2 py-0.5 rounded">Default</span>
                  )}
                  {s.read_only && (
                    <span class="text-xs bg-amber-900/50 text-amber-300 px-2 py-0.5 rounded">Read-only</span>
                  )}
                </div>
      ...
      >
        <p class="text-gray-300 mb-4">
          Remove <strong class="text-white">{deleteTarget?.name}</strong>?
          This only removes the share configuration — files on disk are not deleted.
        </p>
...
function ShareFormModal({ open, share, onClose, onSaved }: {
  ...
  return (
    <Modal open={open} onClose={onClose} title={isEdit ? 'Edit Share' : 'Add Share'}>
      <form onSubmit={handleSubmit} class="space-y-4">
        ...
        <div>
          ...
          <input
            ...
            placeholder="e.g. Documents, Media, Photos"
          />
        </div>

        <div>
          <label class="block text-sm font-medium text-gray-300 mb-1">Host Path</label>
          <input
            type="text"
            value={hostPath}
            ...
            placeholder="e.g. D:\Documents or /mnt/media"
          />
          <p class="text-xs text-gray-500 mt-1">Absolute path to an existing directory on the server</p>
        </div>

        <div>
          <label class="block text-sm font-medium text-gray-300 mb-1">Icon hint</label>
          <input
            type="text"
            value={icon}
            onInput={e => setIcon((e.target as HTMLInputElement).value)}
            class="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-white focus:outline-none focus:ring-2 focus:ring-brand-500"
            placeholder="folder, hdd, film, music (optional)"
          />
        </div>
        ...
        <label class="flex items-center gap-3 cursor-pointer">
          ...
          />
          <span class="text-sm text-gray-300">Read-only (clients cannot upload or modify files)</span>
        </label>
        ...
        <div class="flex justify-end gap-3 pt-2">
          ...
          >
            {saving ? 'Saving...' : isEdit ? 'Save Changes' : 'Add Share'}
          </button>
        </div>
      </form>
    </Modal>
